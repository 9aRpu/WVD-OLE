...
Start-Process -FilePath "$LocalPath\OneDriveSetup.exe" -ArgumentList "/uninstall" -Wait

cmd.exe /c 'REG ADD "HKLM\Software\Microsoft\OneDrive" /v "AllUsersInstall" /t REG_DWORD /d 1 /reg:64'

Start-Process -FilePath "$LocalPath\OneDriveSetup.exe" -ArgumentList "/allusers" -Wait
